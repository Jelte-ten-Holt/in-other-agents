 ...
 *
 * Accepts a registration request, validates the metadata, asks Passport
 * to persist a new authorization-code grant client, and returns the
 * issued credentials per §3.2.1. Every registration dispatches
 * `DynamicClientRegistered` (which the bundled AgentLogSubscriber logs).
 *
...
final class DynamicClientRegistrationController
{
    ...
    /**
     * When `agents.auth.oauth.dcr.initial_access_token` is set, RFC 7591 §3
     * authenticated registration is enforced. Empty config = open
     * registration. Non-empty expected value is compared in constant time.
     */
    private function hasValidInitialAccessToken(Request $request): bool
    {
        $expected = (string) config('agents.auth.oauth.dcr.initial_access_token', '');
    ...
    {
        $cap = (int) config('agents.auth.oauth.dcr.max_clients', 0);
    ...
    private function validateRegistration(Request $request): array
    {
        ...
        $supportedScope = (string) config('agents.auth.oauth.scope', 'agent');
    ...
    /**
     * Strip ASCII control characters (0x00-0x1F + DEL) — they'd corrupt
     * log lines and could smuggle ANSI escapes into operator terminals.
     * Trim, cap at 255, fall back to a placeholder if nothing usable.
     */
